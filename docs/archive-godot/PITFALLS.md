...
- **Un outil de banc ne recopie JAMAIS une constante de gameplay — il la lit.** `power_curve_multi.py` gardait `0.77` en dur pour aligner sa ligne de départ sur l'entrée en overtime du cran III de saturation. Le jour où cette valeur est passée à **0,62** dans `SaturationTable`, le banc a continué de placer le bot à la 10ᵉ minute alors que le jeu déclenchait l'overtime à la 8ᵉ : les runs démarraient **deux minutes à l'intérieur** de l'escalade, contre un joueur de niveau 13, et mouraient toutes en une dizaine de secondes. Le durcissement paraissait spectaculaire — on ne mesurait que le décalage de l'outil. **C'est le pire genre de bug de banc** : il ne produit pas du bruit, mais un résultat net, cohérent et flatteur, que rien ne signale. Parade en place : `read_run_duration_mult()` extrait la valeur de `src/Core/Rules/SaturationTable.cs` et **échoue bruyamment** si elle ne la trouve pas. Étendre ce réflexe à toute constante partagée entre le jeu et un outil.
- **Le protocole `--start-at` + `--saturate-arsenal` donne un arsenal saturé mais un personnage NU** (niveau 1, PV de base) : un joueur qui atteint la même minute y arrive avec plusieurs dizaines de niveaux et deux à trois fois les PV. La difficulté qu'il mesure est une **borne haute**. Elle sert à comparer deux réglages entre eux ; elle ne prédit jamais ce que vaudra une session jouée, et un réglage jugé « impossible » au banc peut être confortable en jeu.
...
- **Un export lancé sur un projet jamais scanné échoue SANS RIEN DIRE** : Godot affiche sa bannière de version, sort en **code 0**, et `build/ChimeraProtocol.exe` garde tranquillement sa date précédente. Aucun message d'erreur, aucun avertissement — on croit avoir exporté et on teste l'ancien binaire. Vécu le 2026-08-02 (`.godot/` absent : l'import suivant a bien affiché `first_scan_filesystem`, preuve que rien n'avait jamais été indexé). **Faire précéder tout export d'un `--headless --path . --import`**, et **toujours vérifier la date de l'exe** après coup (`(Get-Item build\ChimeraProtocol.exe).LastWriteTime`) — c'est la seule chose qui distingue un export réussi d'un export fantôme.
- **La stabilité d'un runtime ne prouve pas sa fraîcheur — un artefact PÉRIMÉ est parfaitement stable.** Vécu le 2026-08-03 sur la release **1.26.0** : `release_itch.ps1` a sondé `Wait-DirStable` **avant** que `dotnet publish` n'ait commencé à écrire, a trouvé un `data_*/` stable et complet (celui de la release précédente, 190 fichiers, les 4 DLL critiques présentes), et **butler a expédié le binaire de la version d'avant** — sans une erreur, avec un « Publication OK ». Symptôme au sol : le gameplay poussé était le bon (le code n'avait pas changé), mais l'exe s'annonçait **1.25.1** face à un `version.json` à **1.26.0** → bandeau « nouvelle version » **perpétuel** pour les joueurs web. Détecté seulement parce que la date de l'exe (18:05) était **antérieure au bump de version**, et que l'export réel s'est terminé à 18:31, soit *après* le push.
  - Signal dans la sortie : les logs Godot (`first_scan_filesystem`, `dotnet_publish_project`, `savepack`) apparaissent **entrelacés avec la barre de progression de butler**, voire après le tableau `butler status`. Un export qui « parle » pendant le push n'est pas fini.
  - Second signal, plus sûr : `butler` annonce le volume de données fraîches. Le push périmé a rapporté « added 30.64 MiB fresh data », le re-push correct **1,53 MiB** — le vrai delta d'un exe reconstruit à sources identiques est *petit*.
  - Parade en place : `Wait-FreshArtifacts` attend que `ChimeraProtocol.exe` **et** `data_*/ChimeraProtocol.dll` aient un `LastWriteTime ≥ début de l'export` (timeout 1200 s, **Fail dur**), appelé **avant** `Wait-DirStable`. L'ordre compte : fraîcheur d'abord (a-t-on réécrit ?), stabilité ensuite (a-t-on fini d'écrire ?).
  - **Leçon de méthode** : le commentaire d'origine disait « pas de comparaison de timestamp — Godot rend la main avant d'avoir flush, ce qui provoquait un faux échec ». Supprimer la vérification pour tuer un **faux négatif** a créé un **faux positif** bien plus coûteux. La bonne réponse à « le contrôle se déclenche trop tôt » est de l'**attendre**, jamais de le retirer.
  - Réparation : `-SkipExport` re-stage et re-pousse le binaire déjà exporté, sans toucher au tampon `BuildInfo` (le SHA embarqué reste celui du binaire expédié).
- **`--path` n'existe pas sur le binaire EXPORTÉ**, seulement sur l'éditeur : `ChimeraProtocol.exe --headless --path .` abandonne sur « this Godot binary was compiled without support for path overrides ». Pour fumer-tester une build, se placer dans `build/` et lancer `./ChimeraProtocol.exe --headless -- <flags>` — les flags de debug restant après `--`.
