# Chantiers d'équilibrage 1.20 → 1.26 (ère Godot)

Extrait **verbatim** de `CLAUDE.md`, tel qu'il était avant le retrait du moteur Godot (2026-08-10).
Ces sections racontent le raisonnement de chaque chantier — diagnostic, mesure, arbitrage, ce que le
banc pouvait et ne pouvait pas trancher. **Le fond reste valable** (le jeu Unity porte les mêmes
règles) ; **les chemins de fichiers, les flags et les noms de classes cités sont ceux de Godot** et
n'existent plus tels quels. Pour l'état courant : `CLAUDE.md`, `docs/PROJECT_STATE.md`.
