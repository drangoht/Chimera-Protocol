...
- **Ne jamais poser un content margin < 16 sur un cadre « plaque blindée »** (`CardFrame`/`ButtonFrame`/`PopupFrame`). Leur bande fait **16 px** et le liseré d'accent court **de 12 à 16 px du bord** : en dessous, le contenu se dessine PAR-DESSUS le liseré et les rivets d'angle. C'est ce qui donnait un bouton « Choisir » collé à la bordure sur l'écran de sélection de personnage (corrigé le 2026-07-28). Utiliser `UiStyle.PanelContentMargin` (= 16). Deux exceptions légitimes : `CompactFrame` (cadre sans bande, pour les contrôles < 64 px, ex. drapeaux de langue) et une marge **verticale** plus faible quand la hauteur est comptée — la garder alors explicite et commentée, côté par côté (`SetContentMargin(Side.…)`), jamais `SetContentMarginAll`.
