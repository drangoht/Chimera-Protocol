...
- **Content margin d'un cadre « plaque blindée »** (`CardFrame`/`ButtonFrame`/`PopupFrame`) : leur bande fait **16 px** et le liseré d'accent court **de 12 à 16 px du bord**.
  ...
  - Deux exceptions : `CompactFrame` (cadre sans bande, contrôles < 64 px, ex. drapeaux de langue) et une marge **verticale** plus faible quand la hauteur est comptée — la garder alors explicite et commentée, côté par côté (`SetContentMargin(Side.…)`), jamais `SetContentMarginAll`.
