...
**Phase actuelle : libre.** Dernières livraisons : remap clavier + ZQSD par défaut (section Contrôles
des Options, `src/Systems/InputRemap.cs`), visée souris/stick + réticule Lance Vectorielle, fusions
Rayon Vecteur & Voile de Givre (brume de froid + ennemis gelés), 2026-07-05. Version publiée itch :
**1.9.0**. Détail dans `docs/PROJECT_STATE.md`.
...
- Singletons (AutoLoad) : `GameManager`, `XpSystem`, `InventorySystem`, `LevelUpSystem`, `SaveManager`, `MetaProgressionSystem`, `AudioSystem`, `FusionFlash`, `ScreenShake`, `GameSettings`.
