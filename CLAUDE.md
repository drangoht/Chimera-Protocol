...
- **Boss de fin `rusted_core`** (`src/Entities/Boss/`, spawn 13 min) : sprite dédié `rusted_core_frames.tres`, `_sprite.Scale = 1.8` posé en code (boss massif). HP 1600, XP 500. Salves radiales de 12 `EnemyBullet` /2.5 s + ondes de choc /3.5 s. Mort (`FinishDeath`) → 3 ondes de choc + death burst + **3 `AetherCore`** + `ShowWeaponDrop(3)` + gros shake/hitstop. La grosse XP (500) déclenche aussi plusieurs level-ups = « plus d'armes ».
