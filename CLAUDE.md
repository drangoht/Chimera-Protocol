...
- **Interroger un fichier trop gros pour être lu** → **MCP local** `mcp__local-llm__local_digest` (un ou plusieurs fichiers → une synthèse) / `local_map` (même question sur N fichiers, un résultat chacun). Outils **différés** : `ToolSearch` d'abord. Le serveur lit les fichiers côté LM Studio — **le contenu brut n'entre jamais en contexte**. Mesuré le 2026-08-02 sur `docs/TEST_REPORT.md` : **83 173 tokens lus en local → 675 renvoyés**, ~82 500 économisés.
  - **Les cibles** (rien d'autre ne le justifie) : `docs/TEST_REPORT.md` (~290 Ko) · `docs/GDD.md` (~200 Ko) · `docs/DEVLOG.md` · `docs/PITFALLS.md` · `localization/ui.csv` · `data/*.json` · `user://boss_ttk.log`.
  - ⚠ **Lent** : ~6-7 min pour 290 Ko (13,5 tok/s, map-reduce). Le lancer et continuer à travailler — l'appel bascule seul en tâche de fond au-delà de 120 s. Prévoir `max_tokens` large (900 a été **tronqué** ; viser 1500-2500 pour un inventaire).
...
quand une phase se termine, relire les agents qu'elle concerne (passe faite le 2026-08-02).
